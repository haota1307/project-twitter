...
// Để fix lỗi cần cài thêm cross-env
const env = process.env.NODE_ENV
const envFilename = `.env.${env}`
if (!env) {
  console.log(`Bạn cần cung cấp biến môi trường NODE_ENV (Ví dụ: development, production)`)
  console.log(`Phát hiện biến NODE_ENV = ${env}`)
  process.exit(1) //cho dừng app lại ngay
}
console.log(`Phát hiện NODE_ENV = ${env}, vì thế app sẽ dùng file môi trường là ${envFilename}`)
if (!fs.existsSync(path.resolve(envFilename))) {
  console.log(`Không tìm thấy file môi trường ${envFilename}`)
  console.log(`Lưu ý: App không dùng file .env. Ví dụ môi trường là develop thì app sẽ dùng file .env.development`)
